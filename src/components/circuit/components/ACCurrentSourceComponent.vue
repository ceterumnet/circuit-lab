<template>
  ...
  >
    ...
    <!-- Professional AC Current Source Symbol using matching SVG -->
    <v-group
      :config="{
        x: 0,
        y: 0,
    ...
    >
      ...
      <!-- AC Current Source Symbol Content -->
      <!-- Current flow arrow pointing from positive to negative -->
      <v-line
        :config="{
          points: [-10, -3, 14, -3],
          stroke: isSelected ? '#ff4d4d' : isHighlighted ? '#ffc107' : '#333',
      ...
      <!-- Small sine wave below arrow to indicate AC -->
      <v-line
        :config="{
          points: [-8, 4, -6, 2, -4, 4, -2, 6, 0, 4, 2, 2, 4, 4, 6, 6, 8, 4],
          stroke: isSelected ? '#ff4d4d' : isHighlighted ? '#ffc107' : '#333',
          strokeWidth: 1,
        }"
      />
...
const acParametersLabel = computed(() => {
  ...
  let amplitudeStr = `${amplitude}A`
  if (Math.abs(amplitude as number) >= 1) {
    amplitudeStr = `${amplitude}A`
  } else if (Math.abs(amplitude as number) >= 0.001) {
    amplitudeStr = `${(amplitude as number) * 1000}mA`
  } else {
    amplitudeStr = `${(amplitude as number) * 1000000}μA`
  }
